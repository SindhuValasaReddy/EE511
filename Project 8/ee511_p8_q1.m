...
% Estimating mean and variance using Stratified Sampling
K = 20; Nij = N/K;                               % Stratified sampling
for i = 1:K
    for j = 1:K
...
ql = 0;
qu = 1;
fun = @(p,q) exp(5.*abs(p-5) + 5.*abs(q-5));   % Theoretical mean
...
V=(b-a)*(b-a)*cos(pi + 5*(a + (b-a)*X12) + 5*(a + (b-a)*X22)); % Simple Monte Carlo
display('Mean and Variance using simple Monte Carlo');
disp(mean(V));
disp(2*std(V)/sqrt(N));
...
ql = -1;
qu = 1;
fun1 = @(p,q) cos(pi + 5*p + 5*q);             % Theoretical mean
c = integral2 (@(p,q)fun1(p,q),pl,pu,ql,qu);
disp('Theoretical integral value');
disp(c);